...
self.addEventListener('fetch',e=>{
  ...
  if(e.request.mode==='navigate'||u.pathname.endsWith('.html')){ // network first, so every deploy shows up
    e.respondWith(fetch(e.request).then(r=>{ const c=r.clone(); caches.open(V).then(x=>x.put(e.request,c)); return r; }).catch(()=>caches.match(e.request).then(r=>r||caches.match('app.html')))); return; }
